...
#Output -> None
def addScholarship(docdirect, name, amount, deadline, ava, dir_link, description, contact_info, binary):
    docdirect.add({
    u'Name' : name,
    u'Amount' : amount,
    u'Deadline' : deadline,
...
try:
    ...
    ################ RECENTLY ADDED ##########################  
    #For creating indexing table#
    #Maybe more ideal way to do it?, just copy and past of later loops to retrieve sub categoires
    refList = []
    num = 0
    for x in range(0, len(L1_link)):
        ...
        L2_title = scraping_levels(level2_tbl) 
